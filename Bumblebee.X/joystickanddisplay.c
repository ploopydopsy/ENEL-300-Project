...
int main(void) {
    ...
    while (1) {
        ...
        uint8_t scaled_value = (uint8_t)((adc_value * 255UL) / 4095UL);
        
        // Update the display with the scaled value (0-255)
...
void adc_init(void) {
    ...
    // Note: We're keeping 12-bit resolution and scaling in software
    // rather than switching to 8-bit resolution
}
