...
void timer_init(void) {
    ...
    TCA0.SINGLE.CTRLA = TCA_SINGLE_CLKSEL_DIV4_gc;  // 4MHz / 4 = 1MHz
...
void pwm_outputs_init(void) {
    ...
    // Configure TCB0 for motor PWM output
    TCB0.CTRLA = TCB_CLKSEL_CLKDIV2_gc;  // 2MHz timer
    TCB0.CTRLB = TCB_CNTMODE_PWM8_gc | TCB_CCMPEN_bm;  // 8-bit PWM mode
    TCB0.CCMP = 0x8000;  // Start at 50% duty cycle
    TCB0.CTRLA |= TCB_ENABLE_bm;
    
    // Configure TCB1 for servo PWM output
    TCB1.CTRLA = TCB_CLKSEL_CLKDIV2_gc;  // 2MHz timer
    TCB1.CTRLB = TCB_CNTMODE_PWM8_gc | TCB_CCMPEN_bm;  // 8-bit PWM mode
...
void rc_inputs_init(void) {
    ...
    PORTD.PIN2CTRL = PORT_ISC_BOTHEDGES_gc | PORT_PULLUPEN_bm;  // Both edges interrupt
    PORTD.PIN3CTRL = PORT_ISC_BOTHEDGES_gc | PORT_PULLUPEN_bm;
    PORTD.PIN4CTRL = PORT_ISC_BOTHEDGES_gc | PORT_PULLUPEN_bm;
}
