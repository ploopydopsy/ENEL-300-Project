...
// Include NRF24L01 library (keep your existing NRF24L01 code)
#ifndef NRF24L01_H
#define NRF24L01_H
// Keep all the existing NRF24L01 definitions
// (This part is not modified as it's the same in your working code)
// ...
#endif
...
#define PAYLOAD_SIZE 3  // Joystick X, Joystick Y, Button state
...
int main(void) {
    ...
    // Set static payload width to 3 bytes (X, Y, button) 
    ...
    // Test servo - make sure it moves to center position
    servo_position = 1500;  // center position
    
    // Enable global interrupts
    sei();
    
    // Initial display showing "0" to indicate waiting for data
    display_value(0);
    
    uint8_t prev_button_value = 0;
    uint8_t reception_timeout = 0;
    ...
    while(1) {
        ...
        if (receive_status == OPERATION_DONE) {
            ...
            // Display joystick X value (0-255) on the 4-digit display
            display_value(rx_payload[0]);
            
            // Process joystick X data (for servo control)
            joystick_x = rx_payload[0];
            update_servo_position(joystick_x);
        
            // Process button state and toggle LED if button is pressed
            uint8_t current_button_value = rx_payload[2];
        
            if (current_button_value && !prev_button_value) {
                led_state = !led_state;
                if (led_state) {
            ...
            // Increment timeout counter if no reception
            reception_timeout++;
            
            // If no reception for a long time, display error code
            if (reception_timeout > 100) {
                // Display error code to indicate no signal
                // Flash the display with "Err" or similar
                error_count++;
                
                // Attempting to recover connection
                if (error_count > 10) {
                    // Try to reinitialize radio parameters
...
void servo_init(void) {
    // Configure TCA0 for servo control (1µs resolution at 4MHz)
    TCA0.SINGLE.CTRLA = TCA_SINGLE_CLKSEL_DIV16_gc;  // Prescaler 16 (4MHz/16 = 250kHz), 4µs period
...
void update_servo_position(uint8_t joystick_value) {
    // Map joystick value (0-255) to servo pulse width (1000-2000 µs)
